  ...
  >
    <div className="flex items-center gap-8">
      <div 
        ...
        onClick={onHome}
      >
        <span className="font-black text-2xl tracking-tighter text-brand-dark flex items-center">
          pan<span className="text-brand-primary">i</span>
        </span>
      </div>
...
const TrustSection = () => (
  <div className="bg-slate-50 py-8 px-6 border-b border-brand-border">
...
const JobCard = ({ job, onClick }: { job: Job; onClick: () => void }) => (
  ...
  >
    <motion.div 
      ...
      className="w-20 h-20 rounded-2xl border border-zinc-100 overflow-hidden shrink-0 bg-zinc-50 flex items-center justify-center shadow-sm"
    >
      <img 
        src={job.logo} 
        alt={job.company} 
    ...
    <div className="flex-1">
      <div className="flex flex-wrap items-center gap-4 mb-3">
        <h3 className="text-2xl font-bold text-zinc-900 group-hover:text-brand-primary transition-colors">{job.title}</h3>
        <motion.span 
          whileHover={{ scale: 1.1 }}
          className="badge bg-rose-50 text-brand-primary px-3 py-1 rounded-full text-xs font-bold border border-rose-100"
        >
          {job.type}
        </motion.span>
      </div>
      <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-zinc-500 font-medium mb-6">
        <span className="flex items-center gap-2"><Building2 size={18} className="text-brand-primary" /> {job.company}</span>
        <span className="flex items-center gap-2"><MapPin size={18} className="text-zinc-400" /> {job.location}</span>
    ...
    <div className="flex flex-col justify-between items-end gap-6">
      <motion.button 
        whileHover={{ scale: 1.3, color: '#e11d48', rotate: 15 }}
        whileTap={{ scale: 0.8 }}
        className="text-zinc-300 transition-colors p-2 hover:bg-rose-50 rounded-full"
      >
        <Bookmark size={24} />
      </motion.button>
      <div className="flex flex-col items-end">
        <span className="text-xs font-bold text-zinc-400 mb-3 uppercase tracking-widest">{job.postedAt}</span>
        <motion.button 
          animate={{ 
            boxShadow: [
          ...
          whileTap={{ scale: 0.95 }}
          className="btn-primary py-3 px-8 text-sm font-bold shadow-lg shadow-rose-100"
        >
          Apply Now
        </motion.button>
      </div>
  ...
  >
    ...
    <div className="p-5 flex-1 flex flex-col">
      <div className="flex items-center gap-2.5 mb-4">
        <img src={gig.avatar} alt={gig.freelancer} className="w-7 h-7 rounded-full border border-zinc-100" referrerPolicy="no-referrer" />
        <span className="font-medium text-sm text-zinc-700">{gig.freelancer}</span>
      </div>
      <h3 className="text-base font-medium text-zinc-900 mb-4 line-clamp-2 group-hover:text-brand-primary transition-colors leading-snug">{gig.title}</h3>
      <div className="mt-auto pt-4 border-t border-zinc-50 flex items-center justify-between">
        <div className="flex items-center gap-1 text-sm">
          <Star size={14} className="fill-yellow-400 text-yellow-400" />
          <span className="font-semibold text-zinc-900">{gig.rating}</span>
          <span className="text-zinc-400 text-xs">({gig.reviewsCount})</span>
        </div>
        <div className="text-right">
          <span className="text-[10px] uppercase tracking-wider text-zinc-400 block font-semibold">Starting at</span>
          <span className="font-bold text-zinc-900">{gig.startingPrice}</span>
        </div>
      </div>
    </div>
  </motion.div>
          ...
          >
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="bg-rose-50 p-2 rounded-lg text-brand-primary">
                  <Sparkles size={20} />
                </div>
                <div>
                  <h3 className="font-bold text-zinc-900">paniBot Assistant</h3>
                  <p className="text-[10px] text-zinc-400 uppercase tracking-widest font-bold">AI Career Coach</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-zinc-50/30">
              {messages.length === 0 && (
                <div className="text-center py-12 px-6">
                  ...
                  <h4 className="font-bold text-zinc-900 mb-2">How can I help today?</h4>
                  <p className="text-sm text-zinc-500 leading-relaxed">Ask me about resume optimization, interview strategies, or finding the perfect gig.</p>
                </div>
              )}
              {messages.map((msg, i) => (
            ...
            <div className="p-6 border-t border-slate-100 bg-white">
              <div className="flex gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-100">
                ...
                <button 
                  onClick={handleSend}
                  className="bg-brand-primary text-white p-2.5 rounded-lg shadow-lg shadow-rose-200 hover:bg-brand-secondary transition-colors"
                >
                  <ArrowRight size={18} />
                </button>
              </div>
      ...
      <>
        ...
        >
          <div className="relative h-48 bg-brand-primary/5 p-8 flex items-end">
            <button 
              onClick={onClose}
              className="absolute top-6 right-6 p-2 bg-white/80 hover:bg-white rounded-full shadow-sm text-zinc-400 hover:text-zinc-900 transition-all"
            >
              <X size={20} />
            </button>
            <div className="flex items-center gap-6">
              <div className="w-24 h-24 bg-white rounded-2xl shadow-xl border border-zinc-100 flex items-center justify-center p-4">
                <img src={job.logo} alt={job.company} className="w-full h-full object-contain" referrerPolicy="no-referrer" />
              </div>
              <div className="mb-2">
                <h2 className="text-3xl font-black text-zinc-900 tracking-tight">{job.title}</h2>
          ...
          <div className="flex-1 overflow-y-auto p-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
              <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                <p className="text-[10px] uppercase tracking-widest font-bold text-zinc-400 mb-1">Location</p>
              ...
              <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                <p className="text-[10px] uppercase tracking-widest font-bold text-zinc-400 mb-1">Salary Range</p>
                <div className="flex items-center gap-2 text-zinc-900 font-bold">
              ...
              <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                <p className="text-[10px] uppercase tracking-widest font-bold text-zinc-400 mb-1">Job Type</p>
                <div className="flex items-center gap-2 text-zinc-900 font-bold">
            ...
            <div className="prose prose-zinc max-w-none">
              <h4 className="text-xl font-bold text-zinc-900 mb-4">About the role</h4>
              <p className="text-zinc-600 leading-relaxed mb-8">{job.description}</p>
              
              <h4 className="text-xl font-bold text-zinc-900 mb-4">Requirements & Skills</h4>
              <div className="flex flex-wrap gap-3 mb-8">
                {job.tags.map(tag => (
                  <span key={tag} className="px-4 py-2 bg-rose-50 text-brand-primary rounded-xl text-sm font-bold border border-rose-100">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
            ...
            <div className="flex gap-4">
              ...
              <button className="btn-primary px-12 py-4 shadow-xl shadow-rose-200">
                Apply Now
              </button>
            </div>
          </div>
      ...
      <>
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
              <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                <p className="text-[10px] uppercase tracking-widest font-bold text-zinc-400 mb-1">Rating</p>
                <div className="flex items-center gap-2 text-zinc-900 font-bold">
                  <Star size={16} className="fill-yellow-400 text-yellow-400" />
                  {gig.rating} <span className="text-zinc-400 font-medium">({gig.reviewsCount} reviews)</span>
                </div>
              </div>
              <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                <p className="text-[10px] uppercase tracking-widest font-bold text-zinc-400 mb-1">Starting Price</p>
                <div className="flex items-center gap-2 text-zinc-900 font-bold">
                  <DollarSign size={16} className="text-emerald-500" />
                  {gig.startingPrice}
                </div>
              ...
              <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                <p className="text-[10px] uppercase tracking-widest font-bold text-zinc-400 mb-1">Category</p>
                <div className="flex items-center gap-2 text-zinc-900 font-bold">
            ...
            <div className="prose prose-zinc max-w-none">
              <h4 className="text-xl font-bold text-zinc-900 mb-4">Service Description</h4>
              ...
              <h4 className="text-xl font-bold text-zinc-900 mb-4">What's included</h4>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 list-none p-0">
                {['High-resolution files', 'Commercial use license', 'Unlimited revisions', 'Source files included'].map(item => (
                  <li key={item} className="flex items-center gap-3 text-zinc-600 font-medium">
          ...
          <div className="p-8 border-t border-zinc-100 bg-zinc-50/50 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button className="p-4 text-zinc-400 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all">
                <Bookmark size={24} />
              </button>
              <button className="px-8 py-4 text-zinc-600 font-bold hover:bg-zinc-100 rounded-2xl transition-all">
                Contact Seller
              </button>
            </div>
            <button className="btn-primary px-12 py-4 shadow-xl shadow-rose-200">
        ...
        {!isLoaded && (
          ...
          >
            <div className="flex items-center gap-1.5">
              <span className="font-black text-4xl tracking-tighter text-brand-dark flex items-center">
                pan<span className="text-brand-primary">i</span>
              </span>
            </div>
            <div className="mt-8 w-48 h-1 bg-zinc-100 rounded-full overflow-hidden">
              <motion.div 
                initial={{ x: '-100%' }}
                animate={{ x: '100%' }}
      ...
      >
        ...
        <footer className="bg-white px-6 py-16 border-t border-brand-border">
          ...
          <div className="pt-8 border-t border-brand-border flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="flex items-center gap-6">
              <span className="font-black text-2xl tracking-tighter text-brand-light flex items-center">
                pan<span className="text-brand-primary">i</span>
              </span>
